//! Transformation-engine tests, ported verbatim from the upstream Go gold-standard suite
//! `bamboo_test.go`. These run against the public `bamboo-core` API as an external consumer.

use bamboo_core::{flag, mode, new_engine, parse_builtin_input_method, BambooEngine, IEngine};
